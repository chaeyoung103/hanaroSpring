...
public class UserAdminController {
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "403", description = "관리자 계정이 아님", content = @Content(schema = @Schema(implementation = BaseErrorResponse.class))),
		@ApiResponse(responseCode = "404", description = "존재하지 않는 계정", content = @Content(schema = @Schema(implementation = BaseErrorResponse.class)))
	})
	@PostMapping("/signin")
	...
	@Operation(summary = "전체 회원 목록 조회", description = "모든 사용자의 정보를 조회합니다.")
	@ApiResponses(value = {
	...
	@Tag(name = "[관리자] 회원관리", description = "회원 관리 기능")
	@Operation(summary = "특정 회원 삭제", description = "사용자 ID로 회원을 삭제합니다.")
	@ApiResponses(value = {
